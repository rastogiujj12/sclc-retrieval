...
class ChunkSizePilotConfig(BaseModel):
    chunk_sizes: list[int] = Field(default_factory=lambda: [128, 256, 512])
    selection_split: str = "validation"
    primary_metric: str = "evidence_paragraph_recall_at_token_budget_1024"
    secondary_metric: str = "complete_evidence_at_token_budget_2048"
    robustness_metrics: list[str] = Field(
        default_factory=lambda: ["average_precision", "ndcg_at_5", "recall_at_5"]
    )
    practical_equivalence_margin: float = 0.01

    @model_validator(mode="after")
    def validate_pilot(self) -> "ChunkSizePilotConfig":
        if not self.chunk_sizes or any(size <= 0 for size in self.chunk_sizes):
            raise ValueError("pilot.chunk_sizes must contain positive integers")
        if self.chunk_sizes != sorted(set(self.chunk_sizes)):
            raise ValueError("pilot.chunk_sizes must be sorted and unique")
        if not 0 <= self.practical_equivalence_margin < 1:
            raise ValueError("pilot.practical_equivalence_margin must be in [0, 1)")
        if not self.selection_split:
            raise ValueError("pilot.selection_split cannot be empty")
        return self
...
class AppConfig(BaseModel):
    ...
    pilot: ChunkSizePilotConfig = ChunkSizePilotConfig()
    ...
    def validate_pipeline_depths(self) -> "AppConfig":
        ...
        unsupported_pilot_sizes = set(self.pilot.chunk_sizes).difference(
            self.chunking.supported_chunk_sizes
        )
        if unsupported_pilot_sizes:
            raise ValueError(
                "pilot.chunk_sizes must be included in chunking.supported_chunk_sizes; "
                f"unsupported: {sorted(unsupported_pilot_sizes)}"
            )

        valid_pilot_metrics = {
            self.evaluation.primary_metric,
            *self.evaluation.bootstrap_metrics,
            "average_precision",
            "reciprocal_rank",
            "r_precision",
            "full_ndcg",
        ...
        for cutoff in self.evaluation.cutoffs:
            for prefix in (
                "ndcg_at_",
                "precision_at_",
                "strict_precision_at_",
                ...
                valid_pilot_metrics.add(f"{prefix}{cutoff}")
        for budget in self.evaluation.token_budgets:
            for prefix in (
                "evidence_paragraph_recall_at_token_budget_",
                "evidence_span_coverage_at_token_budget_",
                "complete_evidence_at_token_budget_",
            ):
                valid_pilot_metrics.add(f"{prefix}{budget}")

        requested_pilot_metrics = {
            self.pilot.primary_metric,
            self.pilot.secondary_metric,
            *self.pilot.robustness_metrics,
        }
        unsupported_pilot_metrics = requested_pilot_metrics.difference(
            valid_pilot_metrics
        )
        if unsupported_pilot_metrics:
            raise ValueError(
                "Pilot metrics must be generated by the configured evaluation cutoffs "
                "or token budgets; unsupported: "
                f"{sorted(unsupported_pilot_metrics)}"
            )
        return self
